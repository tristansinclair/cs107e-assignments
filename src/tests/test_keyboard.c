...
static void test_keyboard_sequences(void)
{
    ...
    while (1)
    {
        ...
        if (action.keycode == ESC_SCANCODE)
            break;
    }
...
static void test_keyboard_events(void)
{
    ...
    while (1)
    {
        ...
        if (evt.action.keycode == ESC_SCANCODE)
            break;
    }
...
static void test_keyboard_assert(void)
{
    ...
    printf("\nPress the Caps Lock and type 'T'\n");
    ch = keyboard_read_next();
    assert(ch == 'T');

    printf("\nLeave the Caps Lock pressed, hold down the shift and type 'A'\n");
    ch = keyboard_read_next();
    assert(ch == 'A');
    ...
    printf("\nTest symbols paired w/ numbers 1, 2, 3...8, 9, 0 (testing !, @, # ...)\n");
    char symbols[] = {'!', '@', '#', '$', '%', '^', '&', '*', '(', ')'};
    for (int i = 0; i < 10; i++)
    {
        printf("\nHold shift, and hit the number %d ('%c')\n", (i + 1) % 10, symbols[i]);
        ch = keyboard_read_next();
        assert(ch == symbols[i]);
    }
}
